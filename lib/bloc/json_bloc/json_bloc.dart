  ...
  JsonBloc() : super(JsonInitial()) {
    ...
    on<GetProfileList>((event, emit) async {
      try {
        ...
        emit(JsonLoaded(jsonProfile, null, null, null, null));
      } on NetworkError {
    ...
    on<GetProductMostPopularList>((event, emit) async {
      try {
        ...
        emit(JsonLoaded(null, mList, null, null, null));
      } on NetworkError {
        emit(JsonError("Couldn't fetch data. Is the device online?"));
      }
    });
    on<GetProductUserMostVisitList>((event, emit) async {
      try {
        ...
        emit(JsonLoaded(null, null, mList, null, null));
      } on NetworkError {
        emit(JsonError("Couldn't fetch data. Is the device online?"));
      }
    });
    on<GetNearestTokoList>((event, emit) async {
      try {
        ...
        emit(JsonLoaded(null, null, null, mList,null));
      } on NetworkError {
        emit(JsonError("Couldn't fetch data. Is the device online?"));
      }
    });
    on<GetDetailProductList>((event, emit) async {
      try {
        ...
        emit(JsonLoaded(null, null, null, null,mList,));
      } on NetworkError {
        emit(JsonError("Couldn't fetch data. Is the device online?"));
      }
    });
